...
## 30. Dedicated instructor discovery routes and server-side session filtering

- **Chose:** Expose instructor class discovery under dedicated read-only endpoints (`GET /instructor/classes` and `GET /instructor/classes/:classId/sessions`) protected by `authorize('INSTRUCTOR')`. Active class discovery returns all non-archived classes in the studio projected with safe card fields (`id`, `title`, `description`, `discipline`). "View My Sessions" filters sessions directly in SQL by `class_id` and instructor assignment (`primary_instructor_id = user.id OR instructor_id IN session_co_instructors`), projecting them through `toInstructorSessionResponse()`.
- **Rejected:** Modifying administrative `/classes/*` routes to allow instructor access; filtering active classes by instructor assignment; loading all class sessions into Node.js and filtering in memory.
- **Why:** Administrative `/classes/*` endpoints must remain strictly staff-only to enforce the requirement that instructors cannot create, edit, or archive classes. Instructors need studio-wide visibility into available classes for discovery, but their session access must remain strictly isolated to sessions they teach or assist.


## 31. Role-scoped dashboard database aggregation and 8-week series CTE
